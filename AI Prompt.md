...
- **FSM Шалыто** — конечные автоматы с именами состояний `noun_Verb`. В switch case не должноы быть много строк кода - используй функции SRP. Компилятор должен предупреждать о появлени состояний, которые не обрабатываются.
