    ...
    mainLoop: while(true) {
        switch(состояние) {
            ...
            case СОСТОЯНИЯ_КОНВЕЙЕРА.ЗагрузкаКонфига:
                ...
                состояние = ns_Error(ns) ? СОСТОЯНИЯ_КОНВЕЙЕРА.Ошибка : СОСТОЯНИЯ_КОНВЕЙЕРА.ОпределениеЯзыка
                break

            case СОСТОЯНИЯ_КОНВЕЙЕРА.ОпределениеЯзыка:
                rwd(language_Detect_Decor, ns)
                состояние = ns_Error(ns) ? СОСТОЯНИЯ_КОНВЕЙЕРА.Ошибка : СОСТОЯНИЯ_КОНВЕЙЕРА.ПоискБлоков
                break

            case СОСТОЯНИЯ_КОНВЕЙЕРА.ПоискБлоков:
                rwd(block_Find_Decor, ns)
                состояние = ns_Error(ns) ? СОСТОЯНИЯ_КОНВЕЙЕРА.Ошибка : СОСТОЯНИЯ_КОНВЕЙЕРА.РазборСтрок
                break

            case СОСТОЯНИЯ_КОНВЕЙЕРА.РазборСтрок:
                rwd(lines_Parse_Decor, ns)
                состояние = ns_Error(ns) ? СОСТОЯНИЯ_КОНВЕЙЕРА.Ошибка : СОСТОЯНИЯ_КОНВЕЙЕРА.Выравнивание
                break

            case СОСТОЯНИЯ_КОНВЕЙЕРА.Выравнивание:
                rwd(alignment_Apply_Decor, ns)
                состояние = ns_Error(ns) ? СОСТОЯНИЯ_КОНВЕЙЕРА.Ошибка : СОСТОЯНИЯ_КОНВЕЙЕРА.ЗаменаТекста
                break

            case СОСТОЯНИЯ_КОНВЕЙЕРА.ЗаменаТекста:
                rwd(text_Replace_Decor, ns)
                состояние = ns_Error(ns) ? СОСТОЯНИЯ_КОНВЕЙЕРА.Ошибка : СОСТОЯНИЯ_КОНВЕЙЕРА.Завершено
                break
...
// ============================================================
// ── 8. А2 — АВТОМАТ СКАНЕРА СТРОК (ИСПРАВЛЕНА ВЛОЖЕННОСТЬ) ─
// ============================================================
function а2_АвтоматСканера(raw: string, rules: LanguageRules): ParsedLine {
    ...
    let codeStart      = 0
    let blockEndMarker = ''
    let nestingDepth   = 0  // 🔧 ИСПРАВЛЕНИЕ: переименовано, теперь отслеживает ()[]{}
...
function а4_АвтоматРаспространения(
    ...
    if(состояние === СОСТОЯНИЯ_РАСПРОСТРАНЕНИЯ.Накопление) {применитьМакс()}
}
