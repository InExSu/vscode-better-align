...
function fn_MapNormalize(ctx                        : FSMStateContext): FSMState {
    ctx.a_Columns = map_Normalize(ctx.a_RawMap, ctx.i_MaxSpaces)  // FIX #6: передаём maxSpaces
...
// FIX #6: принимает i_MaxSpaces и применяет ограничение на смещение
export function map_Normalize(a_RawMap: AlignToken[][], i_MaxSpaces: number): AlignColumn[] {
    ...
    for(let i_Col = 0; i_Col < i_ColCount; i_Col++) {
        ...
        let i_MinPos     = Infinity
        let i_MaxPos     = 0
        for(const a_Row of a_RawMap) {
            if(a_Row[i_Col] && a_Row[i_Col].s_Char === s_DominantChar) {
                i_MaxPos = Math.max(i_MaxPos, a_Row[i_Col].i_Pos)
                i_MinPos = Math.min(i_MinPos, a_Row[i_Col].i_Pos)
            }
        }

        // FIX #6: пропускаем колонку если требуемое смещение превышает maxSpaces
        if(i_MaxPos - i_MinPos > i_MaxSpaces) { continue }
