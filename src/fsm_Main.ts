...
function fn_IsInsideBraces(raw: string, i_Pos: number): boolean {
    let i_Depth = 0
...
function fn_IsReturnTypeColon(raw: string, i_Pos: number): boolean {
    let i_Prev = i_Pos - 1
    while(i_Prev >= 0 && raw[i_Prev] === ' ') { i_Prev-- }
    const charBefore = i_Prev >= 0 ? raw[i_Prev] : ''
    if(charBefore !== ')') { return false }
    const afterColon = raw.slice(i_Pos + 1).trimStart()
    return afterColon.startsWith('{') || afterColon.startsWith('|') || afterColon.startsWith('=>')
}
...
export function map_BuildRaw(a_MaskedLines: string[], a_RawLines: string[], a_AlignChars: string[]): AlignToken[][] {
    return a_MaskedLines.map((s_Masked, i_Line) => {
        ...
        let b_InReturnType = isFunctionDecl

        return tokens.filter(tok => {
            switch(tok.s_Char) {
                case ':':
                    if(fn_IsInsideBraces(raw, tok.i_Pos)) { return false }
                    if(b_InReturnType && fn_IsReturnTypeColon(raw, tok.i_Pos)) {
                ...
                case '{':
                    if(tok.i_Pos > 0) {
                        ...
                        while(i_Prev >= 0 && raw[i_Prev] === ' ') { i_Prev-- }
                        if(i_Prev >= 0 && raw[i_Prev] !== ')' && raw[i_Prev] !== ';') {
                            b_InReturnType = false
                        }
                        if(i_Prev >= 0 && (raw[i_Prev] === '=' || /[a-zA-Z0-9]/.test(raw[i_Prev]))) {
                            return false
                        }
                    }
                    return true
